...
async def create_dashboards(projectId: int, data: schemas.CreateDashboardSchema = Body(...),
    ...
    return await dashboards.create_dashboard(project_id=projectId, user_id=context.user_id, data=data)
...
async def get_dashboards(projectId: int, context: schemas.CurrentContext = Depends(OR_context)):
    return {"data": await dashboards.get_dashboards(project_id=projectId, user_id=context.user_id)}
...
async def get_dashboard(projectId: int, dashboardId: int, context: schemas.CurrentContext = Depends(OR_context)):
    data = await dashboards.get_dashboard(project_id=projectId, user_id=context.user_id, dashboard_id=dashboardId)
    if data is None:
...
async def update_dashboard(projectId: int, dashboardId: int, data: schemas.EditDashboardSchema = Body(...),
                     context: schemas.CurrentContext = Depends(OR_context)):
    return {"data": await dashboards.update_dashboard(project_id=projectId, user_id=context.user_id,
...
async def delete_dashboard(projectId: int, dashboardId: int, _=Body(None),
                     context: schemas.CurrentContext = Depends(OR_context)):
    return await dashboards.delete_dashboard(project_id=projectId, user_id=context.user_id, dashboard_id=dashboardId)
...
async def pin_dashboard(projectId: int, dashboardId: int, context: schemas.CurrentContext = Depends(OR_context)):
    return {"data": await dashboards.pin_dashboard(project_id=projectId, user_id=context.user_id, dashboard_id=dashboardId)}
...
async def add_card_to_dashboard(projectId: int, dashboardId: int,
                          ...
                          context: schemas.CurrentContext = Depends(OR_context)):
    return {"data": await dashboards.add_widget(project_id=projectId, user_id=context.user_id, dashboard_id=dashboardId,
                                          data=data)}
...
async def create_metric_and_add_to_dashboard(projectId: int, dashboardId: int,
                                       ...
                                       context: schemas.CurrentContext = Depends(OR_context)):
    return {"data": await dashboards.create_metric_add_widget(project_id=projectId, user_id=context.user_id,
                                                        dashboard_id=dashboardId, data=data)}
...
async def update_widget_in_dashboard(projectId: int, dashboardId: int, widgetId: int,
                               ...
                               context: schemas.CurrentContext = Depends(OR_context)):
    return await dashboards.update_widget(project_id=projectId, user_id=context.user_id, dashboard_id=dashboardId,
...
async def try_card_issues(projectId: int, data: schemas.CardSchema = Body(...),
                    context: schemas.CurrentContext = Depends(OR_context)):
    return {"data": await custom_metrics.get_issues(project_id=projectId, user_id=context.user_id, data=data)}


@app.get('/{projectId}/cards', tags=["cards"])
async def get_cards(projectId: int, context: schemas.CurrentContext = Depends(OR_context)):
    return {"data": await custom_metrics.get_all(project_id=projectId, user_id=context.user_id)}


@app.post('/{projectId}/cards', tags=["cards"])
async def create_card(projectId: int, data: schemas.CardSchema = Body(...),
                context: schemas.CurrentContext = Depends(OR_context)):
    return await custom_metrics.create_card(project_id=projectId, user_id=context.user_id, data=data)


@app.post('/{projectId}/cards/search', tags=["cards"])
async def search_cards(projectId: int, data: schemas.SearchCardsSchema = Body(...),
                 context: schemas.CurrentContext = Depends(OR_context)):
    return {"data": await custom_metrics.search_all(project_id=projectId, user_id=context.user_id, data=data)}
...
async def get_card(projectId: int, metric_id: Union[int, str], context: schemas.CurrentContext = Depends(OR_context)):
    ...
    data = await custom_metrics.get_card(project_id=projectId, user_id=context.user_id, metric_id=metric_id)
    if data is None:
...
async def get_card_funnel_issues(projectId: int, metric_id: Union[int, str],
    ...
    data = await custom_metrics.get_funnel_issues(project_id=projectId, user_id=context.user_id, metric_id=metric_id,
                                            data=data)
    if data is None:
        return {"errors": ["custom metric not found"]}
    return {"data": data}
...
async def get_metric_funnel_issue_sessions(projectId: int, metric_id: int, issueId: str,
                                     data: schemas.CardSessionsSchema = Body(...),
                                     context: schemas.CurrentContext = Depends(OR_context)):
    data = await custom_metrics.get_funnel_sessions_by_issue(project_id=projectId, user_id=context.user_id,
...
async def get_card_chart(projectId: int, metric_id: int, request: Request, data: schemas.CardSessionsSchema = Body(...),
                   context: schemas.CurrentContext = Depends(OR_context)):
    data = await custom_metrics.make_chart_from_card(project_id=projectId, user_id=context.user_id, metric_id=metric_id,
                                               data=data)
    return {"data": data}
...
async def update_card(projectId: int, metric_id: int, data: schemas.CardSchema = Body(...),
                context: schemas.CurrentContext = Depends(OR_context)):
    data = await custom_metrics.update_card(project_id=projectId, user_id=context.user_id, metric_id=metric_id, data=data)
    if data is None:
        return {"errors": ["custom metric not found"]}
    return {"data": data}
...
async def update_card_state(projectId: int, metric_id: int,
                      ...
                      context: schemas.CurrentContext = Depends(OR_context)):
    return {
        "data": await custom_metrics.change_state(project_id=projectId, user_id=context.user_id, metric_id=metric_id,
                                            status=data.active)}
...
async def delete_card(projectId: int, metric_id: int, _=Body(None),
                context: schemas.CurrentContext = Depends(OR_context)):
    return {"data": await custom_metrics.delete_card(project_id=projectId, user_id=context.user_id, metric_id=metric_id)}
