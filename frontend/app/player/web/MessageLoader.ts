...
export default class MessageLoader {
  ...
  private createNewParser(shouldDecrypt=true) {
    ...
    return (b: Uint8Array) => decrypt(b).then(b => {
      ...
      //this._sortMessagesHack(fileReader)
      this.setMessagesLoading(false)
    })
  }
    ...
    const fileReadPromise = this.session.domURL && this.session.domURL.length > 0
  ...
  loadDevtools() {
    // load devtools
    // ---> this.state.update({ devtoolsLoading: true }) // BEFORE
    const fileReadPromise = this.session.devtoolsURL.length > 0
      ? loadFiles(this.session.devtoolsURL, this.createNewParser())
      : Promise.reject()
    return fileReadPromise.catch(() =>
